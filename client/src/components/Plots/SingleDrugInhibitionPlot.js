...
class SingleDrugInhibitionPlot extends React.Component {
  ...
  componentDidMount() {
    ...
    monoDrugData.shift();
    const inhibData = monoDrugData.map(item => standarizeData(100 - item.raw_matrix * 100));
    const concData = monoDrugData.map(item => (drugA ? item.concA : item.concB));
    const drugName = drugA ? drugsData[0].name : drugsData[1].name;
    const hoverData = monoDrugData.map(item => `${standarizeData(100 - item.raw_matrix * 100).toFixed(2)}% inhibition (${drugA ? item.concA : item.concB} µM ${drugName})`);
    const data = [{
      type: 'scatter',
      x: concData,
      y: inhibData,
